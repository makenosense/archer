...
public class UploadTransactionData {
    ...
    public UploadTransactionData(SVNRepository repository, List<File> dirList, List<File> fileList,
                                 Map<File, String> repositoryPathMap) throws Exception {
        ...
        for (File dir : this.dirList) {
            SVNNodeKind kind = repository.checkPath(repositoryPathMap.get(dir), -1);
        ...
        for (File file : this.fileList) {
            SVNNodeKind kind = repository.checkPath(repositoryPathMap.get(file), -1);
